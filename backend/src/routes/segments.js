...
router.post('/', async (req, res, next) => {
  try {
    ...
    const customerCount = await Customer.countDocuments({ ...mongoQuery, userId: req.userId });
    const segment = await Segment.findOneAndUpdate(
      { userId: req.userId, name: { $regex: `^${req.body.name}$`, $options: 'i' } },
      { $setOnInsert: { ...req.body, userId: req.userId, customerCount } },
...
router.post('/preview', async (req, res, next) => {
  try {
    ...
    const userQuery = { ...mongoQuery, userId: req.userId };
    const count = await Customer.countDocuments(userQuery);
    const sample = await Customer.find(userQuery).limit(3).select('name email city ltv');
...
router.get('/:id', async (req, res, next) => {
  try {
    const segment = await Segment.findOne({ _id: req.params.id, userId: req.userId }).lean();
...
router.get('/:id/customers', async (req, res, next) => {
  try {
    ...
    const total = await Customer.countDocuments(userQuery);
    const customers = await Customer.find(userQuery)
...
router.delete('/:id', async (req, res, next) => {
  try {
    await Segment.findOneAndDelete({ _id: req.params.id, userId: req.userId });
    res.json({ ok: true });
  } catch (err) { next(err); }
});
