...
const glassButtonVariants = cva(cn(
  ...
  variants: {
    variant: {
      default: cn(
        "bg-white/20 backdrop-blur-xl border border-slate-200 border-white/30 text-white dark:border-slate-800",
      ...
      primary: cn(
        ...
        "backdrop-blur-xl border border-slate-200 border-white/30 text-white dark:border-slate-800",
      ...
      destructive: cn(
        "bg-red-500/30 backdrop-blur-xl border border-slate-200 border-red-400/40 text-red-100 dark:border-slate-800",
  ...
  ({ className, variant, asChild = false, size, glowEffect = false, children, ...props }, ref) => {
    ...
    return (
      <div className="relative inline-block">
        {glowEffect && (
          <div
            className="absolute -inset-1 rounded-xl bg-linear-to-r from-cyan-500/40 via-blue-500/40 to-purple-500/40 blur-lg opacity-70 transition-opacity group-hover:opacity-100" />
        )}
        <Comp
